...
function CopyIcon() {
  return <svg className="copy-action-icon" viewBox="0 0 20 20" fill="none" stroke="currentColor" strokeWidth="1.65" aria-hidden="true"><rect x="6.5" y="3.5" width="9.5" height="11.5" rx="1.5" /><path d="M4 6.5v9a1.5 1.5 0 0 0 1.5 1.5h7" /></svg>;
}
...
type SchemaIconKind = 'table' | 'column' | 'primary-key' | 'foreign-key' | 'index' | 'relationship' | 'storage';

function SchemaIcon({ kind }: { kind: SchemaIconKind }) {
  const common = { fill: 'none', stroke: 'currentColor', strokeWidth: '1.8', strokeLinecap: 'round' as const, strokeLinejoin: 'round' as const };
  const paths = kind === 'table'
    ? <><rect x="3.5" y="3.5" width="17" height="17" rx="1.5" {...common} /><path d="M3.5 9.25h17M9.25 3.5v17" {...common} /></>
    : kind === 'column'
      ? <><rect x="5" y="3.5" width="14" height="17" rx="1.5" {...common} /><path d="M12 3.5v17" {...common} /></>
      : kind === 'primary-key' || kind === 'foreign-key'
        ? <><circle cx="8.2" cy="8.2" r="3.1" {...common} /><path d="m10.45 10.45 7.05 7.05M13.1 13.1l1.75-1.75M15.15 15.15l1.75-1.75" {...common} /></>
        : kind === 'index'
          ? <><path d="M8 5h11M8 12h11M8 19h11" {...common} /><circle cx="4.5" cy="5" r=".8" fill="currentColor" /><circle cx="4.5" cy="12" r=".8" fill="currentColor" /><circle cx="4.5" cy="19" r=".8" fill="currentColor" /></>
          : kind === 'relationship'
            ? <><circle cx="6" cy="6" r="2" {...common} /><circle cx="18" cy="18" r="2" {...common} /><path d="m7.45 7.45 8.1 8.1M12.9 15.55h2.65V12.9" {...common} /></>
            : <><ellipse cx="12" cy="5.5" rx="7" ry="2.5" {...common} /><path d="M5 5.5v7c0 1.4 3.1 2.5 7 2.5s7-1.1 7-2.5v-7M5 12.5v6c0 1.4 3.1 2.5 7 2.5s7-1.1 7-2.5v-6" {...common} /></>;
  return <svg className={`schema-icon schema-icon-${kind}`} data-schema-icon={kind} viewBox="0 0 24 24" aria-hidden="true">{paths}</svg>;
}
...
function InspectorSummary({ table, onOpenQuery }: { table: CatalogTable; onOpenQuery: () => void }) {
  return <div className="inspector-summary"><p>The complete schema is open in the center workspace.</p><dl><div><dt><SchemaIcon kind="table" />Type</dt><dd>{table.kind}</dd></div><div><dt><SchemaIcon kind="column" />Columns</dt><dd>{table.columns.length}</dd></div><div><dt><SchemaIcon kind="storage" />Storage</dt><dd>{table.withoutRowid ? 'Without rowid' : 'Rowid table'}</dd></div></dl><div className="summary-actions"><button className="quiet-button" onClick={onOpenQuery}>Return to query</button></div></div>;
}
