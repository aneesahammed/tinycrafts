...
| CAT-02 | Selected objects expose bounded column/index details, nullable CREATE SQL, and explicit STRICT/rowid metadata without inventing definitions. | `src/engine/protocol.ts` (`schemaSql`, `withoutRowid`, `strict`); `src/App.tsx` (`TableDetails`); `tests/e2e/app.spec.ts` internal-object test | PARTIAL — index origin/partial-expression metadata, generated-column flags, and lazy/failure-isolated details remain open |
...
- CAT-02 follow-up: index origin/partial-expression/generated-column metadata and lazy/failure-isolated detail loading.
