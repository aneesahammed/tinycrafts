...
- API keys are never stored in localStorage. "Remember key" is off by default; when enabled, each provider key is stored only in this browser using WebCrypto and IndexedDB. This protects against casual storage inspection, not same-origin script compromise.
