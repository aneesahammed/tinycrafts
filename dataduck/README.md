...
- Answers questions about the active file with Ask DataDuck, an optional Groq BYOK assistant
...
Ask DataDuck is an optional browser-side assistant for the active CSV or Parquet file. It uses a user-provided Groq API key to turn a natural-language question into a structured analysis plan. DataDuck validates that plan, compiles safe SQL against `active_file`, executes it locally in DuckDB-WASM, and renders the result table/chart in the browser.

Privacy defaults:

- Source file rows are not sent to Groq.
- Current query result rows are not sent to Groq by default.
...
- Groq never returns executable SQL; DataDuck compiles SQL from a validated plan.
